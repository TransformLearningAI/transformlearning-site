  ...
  return (
    <div className="mt-1">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-white/40">{titles[level - 1]}</span>
      </div>
...
export default function Sidebar({ profile, enrollments = [], open, onClose, theme }) {
  ...
  const mainNav = isFaculty ? [
    ...
    { href: '/analytics', label: 'Analytics', icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 14V9M6 14V6M10 14V4M14 14V2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
    )},
    { href: '/settings', label: 'Settings', icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.5"/><path d="M8 1v2M8 13v2M1 8h2M13 8h2M3.05 3.05l1.41 1.41M11.54 11.54l1.41 1.41M3.05 12.95l1.41-1.41M11.54 4.46l1.41-1.41" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
    )},
  ] : [
    { href: '/my-progress', label: 'Skill Mapping', icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.5"/><circle cx="8" cy="8" r="2.5" stroke="currentColor" strokeWidth="1.5"/><path d="M8 2v2M8 12v2M2 8h2M12 8h2" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
    )},
    { href: '/learn', label: 'Learn', icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 4l6-2 6 2v8l-6 2-6-2V4z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/><path d="M8 6v8" stroke="currentColor" strokeWidth="1.3"/></svg>
    )},
    { href: '/study-planner', label: 'Study Planner', icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="2" stroke="currentColor" strokeWidth="1.5"/><path d="M2 6h12M6 2v12" stroke="currentColor" strokeWidth="1.3"/></svg>
    )},
    { href: '/sources', label: 'Source Library', icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 2h4l2 2h4a1 1 0 011 1v8a1 1 0 01-1 1H3a1 1 0 01-1-1V3a1 1 0 011-1z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/></svg>
    )},
  ]
    ...
    <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-64 flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {isFaculty && <p className="text-[10px] text-white/40 uppercase tracking-wider">Faculty</p>}
          </div>
      ...
      <div className="px-3 flex-1">
        ...
        {!isFaculty && enrollments.length > 0 && (
          <div className="mt-6">
            <p className="px-3 text-[10px] font-bold text-white/30 uppercase tracking-[0.12em] mb-2">My Classes</p>
      ...
      <div className="px-3 pb-4 space-y-2">
        <button onClick={signOut}
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-white/30 hover:text-white hover:bg-white/5 w-full transition-all">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 2H4a2 2 0 00-2 2v8a2 2 0 002 2h2M11 11l3-3-3-3M6 8h8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          Sign out
        </button>
